...
def test_render_downtrend_ends_red():
    ...
    # PNG and the closing candle must actually render in DOWN's red accent —
    # the direction this whole account posts.
    from src import chart

    class _RecordingRectangle(chart.Rectangle):
        facecolors = []

        def __init__(self, *a, **kw):
            _RecordingRectangle.facecolors.append(kw.get("facecolor"))
            super().__init__(*a, **kw)
    ...
    monkeypatch_rectangle = chart.Rectangle
    chart.Rectangle = _RecordingRectangle
    try:
        png = chart._render_png(_c("SLIDE"), hist)
    finally:
        chart.Rectangle = monkeypatch_rectangle

    assert png[:8] == b"\x89PNG\r\n\x1a\n" and len(png) > 10_000
    assert _RecordingRectangle.facecolors, "no body rectangles were recorded"
    assert _RecordingRectangle.facecolors[-1] == chart.DOWN
